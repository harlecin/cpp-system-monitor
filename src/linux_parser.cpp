...
Statistics
LinuxParser::StatParser(int pid) {
  string line;
  string value;
  Statistics stats;
  ...
  if (filestream.is_open()) {
    ...
    while (linestream >> value) {
      if (i == 14) {
        stats.uTime = (std::stol(value));
      }
      if (i == 15) {
        stats.sTime = (std::stol(value));
      }
      if (i == 16) {
        stats.cuTime = std::stol(value);
      }
      if (i == 17) {
        stats.csTime = std::stol(value);
      }
      // Time process started after system boot:
      if (i == 22) {
        stats.startTime = std::stol(value);
      }
      i++;
    }
  }
